...
class TestVisualizationSmoke:
    ...
    def test_biplot_runs(self, pca_model_and_data, tmp_path):
        ...
        fig = plot_biplot(df_pca, pca, feat_names, save_path=str(tmp_path / "biplot.png"))
        import matplotlib.figure
        assert isinstance(fig, matplotlib.figure.Figure)

    def test_loadings_heatmap_runs(self, pca_model_and_data, tmp_path):
        pca, _, feat_names = pca_model_and_data
        fig = plot_loadings_heatmap(pca, feat_names, n_components=3, save_path=str(tmp_path / "loadings.png"))
        import matplotlib.figure
        assert isinstance(fig, matplotlib.figure.Figure)

    def test_pairplot_runs(self, pca_model_and_data, tmp_path):
        _, df_pca, _ = pca_model_and_data
        g = plot_pairplot_pca(df_pca, n_components=3, save_path=str(tmp_path / "pairplot.png"))
        import seaborn
    ...
    def test_interactive_returns_path(self, pca_model_and_data, tmp_path):
        ...
        path = plot_pca_interactive(df_pca, pca_model=pca, save_path=str(tmp_path / "interactive.html"))
