...
> **Evolución de NEXO:** `Anexo Risk` mantiene la misma app (mapa como protagonista, ALTO RIESGO desbloquea necesidades/ayudas) pero ahora con 4 personas (Javi, Juan, Luis, Vanessa) cubriendo los 4 módulos. Cada persona es dueña **equitativa, sin solapes** de su módulo E2E (ver **único** `docs/equipos/reparto.md` — 5 tareas c/u). **Producto integral:** ningún módulo brilla solo — la demo es `alerta → zona → necesidad → ayuda → mapa`.

Base común montada y ya en `anexo-risk` (a687fda) + Vercel (`api/index.py` + `vercel.json`) con 4 equipos verticales y `anexo-risk` como rama única de integración → PR a `main` (GitHub Flow, `main` bloqueada).
...
- **Archivos:** `backend/modules/necesidades/*`, `frontend/js/core/mapa-necesidades/formulario*`, `necesidadCard.js`, `geocodificacion.js` — ver `docs/equipos/reparto.md` (Persona 3 Luis)
...
- **Archivos:** `backend/modules/alertas/*`, `backend/integrations/gdacs*`, `backend/config.py` (CORS) — ver `docs/equipos/reparto.md` (Persona 1 Javi)
...
- **Archivos:** `backend/modules/voluntariado/*`, `backend/modules/donaciones/*`, `frontend/pages/donaciones.html`, `frontend/js/core/voluntariado-donaciones/**`, `frontend/mocks/ayudas.mock.json` — ver `docs/equipos/reparto.md` (Persona 4 Vanessa)
...
- **Archivos:** `frontend/pages/mapa.html`, `frontend/js/core/mapa-necesidades/mapaNecesidades.js`, `frontend/js/shared/apiClient.js`, `frontend/mocks/*.json` — ver `docs/equipos/reparto.md` (Persona 2 Juan)
